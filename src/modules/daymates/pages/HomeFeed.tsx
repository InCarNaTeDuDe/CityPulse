...
const fetchFeeds = async (
  user: any,
): Promise<{ activities: Activity[]; tickets: TicketType[] }> => {
  try {
    ...
    console.error("Failed to fetch feeds from Express backend.", e);
  }
    ...
    <div className="px-5 py-6 font-sans animate-pulse">
      ...
      <div className="space-y-4">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="p-5 bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-100 dark:border-neutral-800 space-y-4"
          >
...
export const HomeFeed: React.FC = () => {
  ...
  // Synchronize state directly in render (React 19 pattern to avoid useEffect for syncing props/context)
...
const HomeFeedContent: React.FC<HomeFeedContentProps> = ({ feedPromise }) => {
  ...
  // Resolve Promise directly during render using React 19 use()
  const data = use(feedPromise);
  const activities = data.activities;
  const tickets = data.tickets;

  // UI States
  const [filterMode, setFilterMode] = useState<"all" | "daymates" | "tickets">(
    "all",
  );
  ...
  const handleJoinActivity = (actId: string) => {
    ...
    startActionTransition(async () => {
      try {
        ...
        console.error("Failed to join activity on backend.", e);
        toast("Connection error. Failed to join activity.", "error");
      }
    });
  };
    ...
    startActionTransition(async () => {
      try {
        ...
        if (res.ok) {
          ...
          toast(
            `🎟 Securely bought "${result.ticket.title}"! Escrow protection active.`,
          );
        ...
        console.error("Failed to buy ticket on backend.", e);
        toast("Connection error. Failed to complete purchase.", "error");
        setIsBuyModalOpen(false);
      }
    });
  };
  ...
  // Blended Feed List
  const blendedFeed: Array<{
    type: "daymate" | "ticketswap";
    data: Activity | TicketType;
    timeSort: string;
  }> = [];
    ...
    <div className="px-5 py-6 font-sans max-w-5xl mx-auto">
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          placeholder={
            filterMode === "tickets"
              ? "Search movies or events..."
              : "Search activities, sports, coffee..."
          }
          value={searchQuery}
        ...
        {blendedFeed.length > 0 ? (
          blendedFeed.map((item) => {
            if (item.type === "daymate") {
              ...
              return (
                <Card
                  key={act.id}
                  className="border border-neutral-100 dark:border-neutral-800/80 relative overflow-hidden bg-white dark:bg-neutral-900"
                >
                  {/* Indicator bar */}
                  ...
                  <div className="flex justify-between items-start pl-2">
                    <div className="flex items-center gap-2">
                      <Badge
                        variant="primary"
                        className="bg-emerald-50 dark:bg-emerald-950 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900 text-[10px]"
                      >
                        Day Mates
                      </Badge>
                ...
                >
                  ...
                  <div className="flex justify-between items-start pl-2">
                    <div className="flex items-center gap-2">
                      <Badge
                        variant="warning"
                        className="bg-teal-50 dark:bg-teal-950 text-teal-600 dark:text-teal-400 border border-teal-100 dark:border-teal-900 text-[10px]"
                      >
                        TicketSwap
                      </Badge>
                      <span className="text-[10px] text-neutral-400 font-bold">
                    ...
                    <Badge
                      variant="success"
                      className="bg-emerald-50 dark:bg-emerald-950 text-emerald-600 dark:text-emerald-400 text-[10px] font-extrabold border border-emerald-100"
                    >
                  ...
                  <div className="mt-3 pl-2">
                    ...
                    <div className="flex gap-4 mt-2 bg-neutral-50 dark:bg-neutral-800/50 p-2 rounded-xl border border-neutral-100/50 dark:border-neutral-800/30">
                      <div>
                        ...
                        <p className="text-xs font-bold text-neutral-500 dark:text-neutral-400 line-through">
                          ₹{tkt.originalPrice}
                        </p>
                      </div>
                      <div>
                      ...
                      <div className="ml-auto flex items-center pr-1">
                        <span className="text-[9px] text-neutral-500 dark:text-neutral-400 font-bold">
                          Venue: {tkt.location.substring(0, 15)}...
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          <div className="flex flex-col items-center py-12 text-center text-neutral-400 dark:text-neutral-500 col-span-full">
            ...
            <p className="text-xs font-bold">
              No active local opportunities matching your filters.
            </p>
            <p className="text-[10px] mt-1">
              Be the first to list an activity or sell a ticket!
            </p>
          </div>
        )}
      </div>
          ...
          <div className="flex flex-col gap-4">
              ...
              <div className="mt-4 space-y-2 border-t border-neutral-100 dark:border-neutral-800 pt-3">
                ...
                <div className="flex justify-between text-sm font-extrabold text-neutral-800 dark:text-neutral-200 pt-2 border-t border-dashed border-neutral-200 dark:border-neutral-800">
                  ...
                  <span className="text-emerald-600 dark:text-emerald-400">
                    ₹{selectedTicket.sellingPrice + selectedTicket.connectFee}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 bg-emerald-50 dark:bg-emerald-950/20 p-3 rounded-xl border border-emerald-100 dark:border-emerald-900/50">
              ...
              <p className="text-[10px] text-emerald-700 dark:text-emerald-300 font-medium leading-relaxed">
                Money is protected securely. We only release it once you scan
                successfully. Rating protection included.
              </p>
            </div>
