...
export const ChatsPage: React.FC = () => {
    ...
    try {
      if (user) {
        ...
        if (res.ok) {
          ...
          if (activeChat) {
            const freshActive = data.find(
              (c: Conversation) => c.id === activeChat.id,
            );
            if (freshActive) {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("Failed to send message on backend.", err);
      toast("Connection error. Failed to send message.", "error");
    }
  };
  ...
  return (
    <div className="h-full flex flex-col md:flex-row font-sans select-none relative min-h-[72vh] bg-white dark:bg-neutral-950">
      ...
      <div
        className={`flex flex-col ${activeChat ? "hidden md:flex md:w-80 md:shrink-0 md:border-r md:border-neutral-100 dark:md:border-neutral-900 bg-white dark:bg-neutral-950" : "flex-1"}`}
      >
        {/* Header */}
        <div className="px-5 py-6 border-b border-neutral-50 dark:border-neutral-900 bg-white dark:bg-neutral-950">
          ...
          <p className="text-xs text-neutral-400 dark:text-neutral-500 font-bold mt-0.5">
            Your unified opportunity messaging inbox
          </p>
        </div>
          ...
          {conversations.length > 0 ? (
            conversations.map((conv) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p
                    className={`text-xs mt-1.5 truncate ${conv.unread ? "font-bold text-neutral-800 dark:text-neutral-200" : "text-neutral-500 dark:text-neutral-400"}`}
                  >
                    {conv.lastMessage}
                  </p>
                </div>
            ...
            <div className="text-center py-24 text-neutral-400">
              ...
              <p className="text-xs mt-1">
                Join an activity or buy a ticket to start chatting!
              </p>
            </div>
          )}
        </div>
      ...
      {activeChat ? (
        <div className="flex flex-col flex-1 bg-neutral-50/50 dark:bg-neutral-900/10 absolute inset-0 md:relative md:inset-auto z-40">
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <span
                      className={`text-[8px] font-bold block text-right mt-1.5 ${isMe ? "text-neutral-300 dark:text-neutral-500" : "text-neutral-400 dark:text-neutral-500"}`}
                    >
                      {msg.time}
                    </span>
                  </div>
          ...
          <form
            ...
            className="p-3 bg-white dark:bg-neutral-950 border-t border-neutral-100 dark:border-neutral-900 flex gap-2"
          >
            <input
        ...
        <div className="hidden md:flex flex-col flex-1 items-center justify-center text-neutral-400 dark:text-neutral-500 bg-neutral-50/50 dark:bg-neutral-900/10 p-6">
          <div className="text-center p-8 bg-white dark:bg-neutral-950 rounded-2xl border border-neutral-100 dark:border-neutral-900 shadow-sm max-w-sm">
            ...
            <p className="text-xs mt-1.5 leading-relaxed text-neutral-500">
              Select a chat thread from the left list pane to begin chatting
              securely over escrow transaction ledgers.
            </p>
          </div>
        </div>
      )}
